 ...
 * (2 to 10 connections). SSL is enforced whenever DATABASE_URL points at a non-local host,
 * with self-signed certificate support enabled (rejectUnauthorized: false) to accommodate
 * managed cloud databases such as Render. SQL logging is enabled only in development so
...
const sequelize = new Sequelize(process.env.DATABASE_URL, {
  ...
  dialectOptions: isRemoteDb ? { ssl: { require: true, rejectUnauthorized: false } } : {},
});
